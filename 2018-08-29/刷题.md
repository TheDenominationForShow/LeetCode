...
public:
    void findSecretWord(vector<string>& wordlist, Master& master) {
        Master m(master);
        (Master m(master), for_each(wordlist.begin(), wordlist.end(), [&m, &master](auto &x){if (m.guess(x) == 6) { master.guess(x);}});
    }
};
```
## 6. Z字形变换@2018/06/26
...
### 星叔
```ruby
s.split('').each.inject([[],{"("=>")","{"=>"}", "["=>"]"},true]){|t,i|t[1].include?(i) ?t[0]<<i:(t[2]=false if t[0].length==0||t[1][t[0].pop]!=i);t}.instance_eval{|e|e[2]&&e[0].length==0}
```
### 总结
...
3,stack,数据结构stack 确实可以在这里发挥很大的作用，而我却没想打

##8. 字符串转整数 (atoi)@2018/07/06
###jzf
```c++
class Solution {
public:
    int myAtoi(string str) {
        size_t  ch =0;
        bool flag =true;
        while(ch<str.size() &&isspace(str[ch]))ch++;
        
        if(ch==str.size()) return 0;
         if(str[ch]=='+')
        {
             ch++;
            flag =true;
        ...
        else if(!isdigit(str[ch]))
        {
            return 0;
        }
        long ret=0;
        for(size_t i =ch;i<str.size();i++)
        {
...
public:
    ...
    int romanToInt(string s) {
        int ret =0;
        int tmp1 =translation(s[0]);
        int tmp2;
        for(size_t i=0;i<s.size()-1;i++)
        {
            int flag;
            tmp2 =translation(s[i+1]);
            ret+=(tmp2>tmp1)?-tmp1:tmp1;
            tmp1 =tmp2;
        }
        return ret+=tmp1;
    }
...
```
### 总结
1，这次加不加神器影响非常大，80ms-0ms，有机会研究下怎么回事。
2，分析问题的思路很重要，这几天的问题，如何抽象出满足结果的特征条件，很影响求解。思考的角度和解题思路。
3，抽象与分解，今下午和北河玩的这几道题，将一些在求解过程中要用到的方法步骤，单独写为一个函数。可以节省在可以节省在推导过程中的力气和复杂度。代码也很简洁。
3，几个算法的高级分析技术。
...
public:
    string convert(string s, int numRows) {
        ...
        for(auto vch:vec)
        {
           copy(vch.begin(),vch.end(),back_inserter(retStr));
        }
        return retStr;
    }
};
...
1，string 的+=符号对象是string 而不是char，所以还是蛮蛋疼的
## 104. 二叉树的最大深度@2018/07/09
### 小白
他喵的没有溢出我去
...
public:
    bool canConstruct(string ransomNote, string magazine) {
        for (auto x : ransomNote) {
            string::iterator it = find(magazine.begin(), magazine.end(), x);
            if (it != magazine.end()) {
                swap(*it, (*--magazine.end()));
                magazine.pop_back();
                continue;
            }
            return false;
...
public:
    bool canConstruct(string ransomNote, string magazine) {
        for (auto x : ransomNote) {
            short idx = magazine.find(x);
            if (idx != string::npos) {
                swap(magazine[idx], magazine.back());
                magazine.pop_back();
                continue;
            }
            return false;
...
public:
    ...
    bool canConstruct(string ransomNote, string magazine) {
        bool ret=false;
        if(ransomNote.size()> magazine.size()) return ret;
        if(ransomNote.size()==0) return true;
        
        sort(ransomNote.begin(),ransomNote.end());
        sort(magazine.begin(),magazine.end());
        auto iter =ransomNote.begin();
        
        for(size_t i=0;i<=magazine.size();i++)
        {
           if(magazine[i]== *iter)  iter++;
           if(iter==ransomNote.end()) return true;
        }
...
public:
    ListNode* mergeTwoLists(ListNode* l1, ListNode* l2) {
        ...
        h->next  = q ? q : (p ? p : nullptr);
        return head->next;
    }
};
```
## 88. 合并两个有序数组@2018/07/18
### 北河
```c++
static auto XXX =[]{
    ios::sync_with_stdio(false);
...
*
```c++
std::inplace_merge(nums1.begin(), nums1.begin()+m, std::copy(nums2.begin(), nums2.end(), nums1.begin()+m));
